...
public final class PunishmentImposeWarnCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(PunishmentImposeWarnCommand.class);
    ...
    private static int handleImposeWarn(@NonNull CommandContext<CommandSource> context, PunishmentService punishmentService) {
        ...
        punishmentService.resolveTargetPlayer(targetName)
                .thenAccept(targetOptional -> {
                    ...
                    punishmentService.impose(source, target, PunishmentType.WARN, duration, reason)
                            .thenAccept(result -> {
                                switch (result) {
                                    case PunishmentImposeResult.Imposed imposed -> {
                                        ...
                                        source.sendMessage(StringUtils.deserialize(PunishmentProxyConstants.IMPOSE_SUCCESS_WARN, resolver));
                                    }
                                    ...
                                    case PunishmentImposeResult.AlreadyWarnedForReason ignored ->
                                            source.sendMessage(StringUtils.deserialize(
                                                    PunishmentProxyConstants.ERROR_PLAYER_ALREADY_WARNED_FOR_REASON,
                            ...
                            .exceptionally(throwable -> {
                                LOGGER.error("Failed to warn player {}", target.playerUuid(), throwable);
