...
public final class PunishmentImposeMuteCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(PunishmentImposeMuteCommand.class);
    ...
    private static int handleImposeMute(@NonNull CommandContext<CommandSource> context, PunishmentService punishmentService) {
        ...
        punishmentService.resolveTargetPlayer(targetName)
                .thenAccept(targetOptional -> {
                    ...
                    punishmentService.impose(source, target, PunishmentType.MUTE, duration, reason)
                            .thenAccept(result -> {
                                switch (result) {
                                    case PunishmentImposeResult.Imposed imposed -> {
                                        ...
                                        source.sendMessage(StringUtils.deserialize(PunishmentProxyConstants.IMPOSE_SUCCESS_MUTE, resolver));
                                    }
                                    ...
                                    case PunishmentImposeResult.AlreadyMuted ignored ->
                                            source.sendMessage(StringUtils.deserialize(PunishmentProxyConstants.ERROR_PLAYER_ALREADY_MUTED,
                                                    Placeholder.unparsed("target", target.username())));
                            ...
                            .exceptionally(throwable -> {
                                LOGGER.error("Failed to mute player {}", target.playerUuid(), throwable);
