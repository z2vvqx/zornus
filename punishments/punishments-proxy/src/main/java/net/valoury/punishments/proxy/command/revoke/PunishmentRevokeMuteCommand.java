...
public final class PunishmentRevokeMuteCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(PunishmentRevokeMuteCommand.class);
    ...
    private static int handleRevokeMute(@NonNull CommandContext<CommandSource> context, PunishmentService punishmentService) {
        ...
        punishmentService.resolveTargetPlayer(targetName)
                .thenAccept(targetOptional -> {
                    ...
                    punishmentService.revokeActive(target, PunishmentType.MUTE, source, reason)
                            .thenAccept(result -> {
                                switch (result) {
                                    case PunishmentRevokeResult.Revoked ignored ->
                                            source.sendMessage(StringUtils.deserialize(PunishmentProxyConstants.REVOKE_SUCCESS_MUTE,
                                                    Placeholder.unparsed("target", target.username())));
                                    case PunishmentRevokeResult.PlayerNotMuted ignored ->
                            ...
                            .exceptionally(throwable -> {
                                LOGGER.error("Failed to revoke mute for {}", target.playerUuid(), throwable);
                                source.sendMessage(StringUtils.deserialize(SharedConstants.ERROR_UNEXPECTED));
                                return null;
                            });
                })
