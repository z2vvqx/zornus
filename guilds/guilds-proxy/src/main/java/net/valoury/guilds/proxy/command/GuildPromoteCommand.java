...
public final class GuildPromoteCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(GuildPromoteCommand.class);
    ...
    public static LiteralArgumentBuilder<CommandSource> create(
        ...
        return BrigadierCommand
                .literalArgumentBuilder("promote")
                .executes(context -> {
                    context.getSource().sendMessage(
                            StringUtils.deserialize(GuildProxyConstants.USAGE_PROMOTE));
                ...
                .then(BrigadierCommand
                        ...
                        .executes(context -> handlePromoteMember(context, guildService))
                );
    }

    private static int handlePromoteMember(
        ...
        guildService.promoteMember(sender, targetName)
                .thenAccept(result -> handleResult(sender, result))
                .exceptionally(throwable -> {
                    LOGGER.error("Failed to promote guild member {}", targetName, throwable);
    ...
    private static void handleResult(
        ...
        switch (result) {
            case GuildRankChangeResult.Changed ignored -> {
                // The guild-wide promotion announcement is also the actor's confirmation.
            }
            ...
            case GuildRankChangeResult.PromotionWouldMatchActorRank ignored ->
                    sender.sendMessage(StringUtils.deserialize(
                            GuildProxyConstants.RANK_ERROR_PROMOTION_MATCHES_ACTOR));
            case GuildRankChangeResult.AlreadyHighestRank alreadyHighestRank ->
                    sender.sendMessage(StringUtils.deserialize(
                            GuildProxyConstants.RANK_ERROR_ALREADY_HIGHEST,
                            Placeholder.unparsed("target", alreadyHighestRank.targetName())
                    ));
            case GuildRankChangeResult.AlreadyLowestRank ignored ->
                    sender.sendMessage(StringUtils.deserialize(
